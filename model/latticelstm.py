...
class WordLSTMCell(nn.Module):
    ...
    def forward(self, input_, hx):
        ...
        wh_b = torch.addmm(bias_batch, h_0, self.weight_hh)#[batch_size,4*hidden_size] #torch.addmm=torch.mm(h_0,self.weight_hh)+bias_batch
...
class LatticeLSTM(nn.Module):
    ...
    def forward(self, input, skip_input_list, hidden=None):
        ...
        skip_input = skip_input[0]#这个地方把batch_size去掉，因为它不支持多个batch，所以batch这一纬始终
        hidden_out = []
        memory_out = []
        if hidden:
            (hx,cx)= hidden
        ...
        input_c_list = init_list_of_objects(seq_len)#初始化全为空
        for t in id_list:
            # 用一个RNN拼接
