...
class WordLSTMCell(nn.Module):
    ...
    def forward(self, input_, hx):
        ...
        bias_batch = (self.bias.unsqueeze(0).expand(batch_size, *self.bias.size()))#[batch_size,4*hidden_size]
        wh_b = torch.addmm(bias_batch, h_0, self.weight_hh)#[batch_size,4*hidden_size] #torch.addmm等价于torch.mm(h_0,self.weight_hh)+bias_batch
        wi = torch.mm(input_, self.weight_ih)#[batch_size,4*hidden_size] #torch.mm(input_, self.weight_ih)
        f, i, cc, o = torch.split(wh_b + wi, split_size_or_sections=self.hidden_size, dim=1)#[batch_size,hidden_size]
        f = torch.sigmoid(f)
        i = torch.sigmoid(i)
        cc = torch.tanh(cc)
        c_1 = f*c_0 + i*cc
...
class MultiInputLSTMCell(nn.Module):
    ...
    def forward(self, input_, c_input, hx):
        ...
        if c_num == 0:
            ...
            c_input_var = torch.cat(c_input, 0)#[max_match_num,batch_size,hidden_size]
            ...
            ## alpha  = i concat alpha #增加一个对结果是1的过滤，是1的地方
...
class LatticeLSTM(nn.Module):
    ...
    def forward(self, input, skip_input_list, hidden=None):
        """
            input: variable (batch, seq_len, input_len), batch = 1
        ...
        input = input.transpose(1,0)#[seq_len,batch_size]
        seq_len = input.size(0)
        batch_size = input.size(1)
        assert(batch_size == 1)#这里不支持多个batch的操作
        # skip_input = skip_input[0]#这个地方把batch_size去掉，因为它不支持多个batch，所以batch这一纬始终为1
        hidden_out = []
        memory_out = []
        if hidden:
            (hx,cx)= hidden
        ...
        # input_c_list = init_list_of_objects(seq_len)#初始化全为空#[seq_len,match_num,emb]

        # #起始的句子一定没有词组输入
        # (hx,cx) = self.word_rnn(input[id_list[0]], (hx,cx))
        # hidden_out.append(hx)
        # memory_out.append(cx)

        for t in id_list:
            # create input_c
            input_c=torch.zeros([seq_len,t,self.input_dim])
            # 用一个RNN拼接
            if len(skip_input[t][0])!=0:#第t个词有对应的词组
                matched_num = len(skip_input[t][0])#第t个词匹配到的词组的个数
                # print(skip_input[t][0])
                ...
                ht,ct = self.word_rnn(word_emb, (hx,cx))
                assert(ct.size(0)==len(skip_input[t][1]))
                for idx in range(matched_num):
                    length = skip_input[t][1][idx]
                    if self.left2right:
                        # if t+length <= seq_len -1: 添加gaz信息到input_c_list,是否是list每个元素长度不一样？？？
                        input_c_list[t+length].append(ct[idx,:].unsqueeze(0))#相当于在前面的时候，计算好后面应该加的词组的地方，比如“南京市长江大桥”在计算“南”这个字的时候，这里会把“南京”通过word_rnn后放到“京”的input_c_list中，会把“南京市”通过word_rnn后放到“市”的input_c_list中
                    else:
                        # if t-length >=0:
                        input_c_list[t-length].append(ct[idx,:].unsqueeze(0))
